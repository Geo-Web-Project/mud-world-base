...
struct GeoAnchorComData {
  int32 h;
  string geohash;
}

library GeoAnchorCom {
  FieldLayout constant _fieldLayout =
    FieldLayout.wrap(0x0004010104000000000000000000000000000000000000000000000000000000);

  // Hex-encoded key schema of (bytes32)
  Schema constant _keySchema = Schema.wrap(0x002001005f000000000000000000000000000000000000000000000000000000);
  // Hex-encoded value schema of (int32, string)
  Schema constant _valueSchema = Schema.wrap(0x0004010123c50000000000000000000000000000000000000000000000000000);
   ...
   */
  function get(ResourceId _tableId, bytes32 key) internal view returns (GeoAnchorComData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   * @notice Get the full data.
   */
  function _get(ResourceId _tableId, bytes32 key) internal view returns (GeoAnchorComData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   */
  function get(IStore _store, ResourceId _tableId, bytes32 key) internal view returns (GeoAnchorComData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
  ...
  function set(ResourceId _tableId, bytes32 key, int32 h, string memory geohash) internal {
    ...
    EncodedLengths _encodedLengths = encodeLengths(geohash);
   ...
   */
  function set(ResourceId _tableId, bytes32 key, GeoAnchorComData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.h);

    EncodedLengths _encodedLengths = encodeLengths(_table.geohash);
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(ResourceId _tableId, bytes32 key, GeoAnchorComData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.h);
   ...
   */
  function set(IStore _store, ResourceId _tableId, bytes32 key, GeoAnchorComData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.h);
  ...
  function decodeStatic(bytes memory _blob) internal pure returns (int32 h) {
    h = (int32(uint32(Bytes.getBytes4(_blob, 0))));
  }
  ...
  ) internal pure returns (GeoAnchorComData memory _table) {
   ...
   */
  function encodeLengths(string memory geohash) internal pure returns (EncodedLengths _encodedLengths) {
    // Lengths are effectively checked during copy by 2**40 bytes exceeding gas limits
    unchecked {
      _encodedLengths = EncodedLengthsLib.pack(bytes(geohash).length);
    }
  }
   ...
   */
  function encode(int32 h, string memory geohash) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(h);
